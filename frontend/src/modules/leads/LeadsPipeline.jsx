 ...
 *
 * PWA-1 (P4) additions:
 *   - useIsMobile: on mobile, defaults to List view (Kanban is desktop-first)
 *   - Mobile header collapses to single row with stacked controls
 *   - Stage filter pills (horizontal scroll) on mobile instead of dropdown
 *   - Filter bar stacks vertically on mobile
 *   - Kanban hidden on mobile (list is the primary mobile view)
 ...
 *   - Pattern 26: both panels stay mounted after first activation
 *
 * GPM-1B additions:
...
function ScoreBadge({ score }) {
  ...
  return (
    <span style={{ background: s.bg, color: s.color, padding: '2px 8px', borderRadius: 20, fontSize: 10, fontWeight: 700, fontFamily: ds.fontSyne }}>
      {s.label}
    </span>
  )
}
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        {stage && (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: stage.dot }} />
...
export default function LeadsPipeline({ onOpenLead, onOpenDemoQueue }) {
  ...
  useEffect(() => {
    getPipelineStages().then(data => {
      ...
      if (Array.isArray(cfg) && cfg.length > 0) {
        ...
        const enabled = cfg.filter(s => s.enabled !== false).map(s => ({ key: s.key, label: s.label, dot: DOT[s.key] || '#7A9BAD' }))
        enabled.push({ key: 'lost', label: 'Lost', dot: '#ef4444' }, { key: 'not_ready', label: 'Not Ready', dot: '#6b7280' })
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if (!leadId) return
      setAttentionMap(prev => ({ ...prev, [leadId]: { ...(prev[leadId] ?? {}), unread_messages: 0 } }))
    }
  ...
  // PWA-1: default to 'list' on mobile
  const [viewMode,       setViewMode]       = useState(isMobile ? 'list' : 'kanban')
  const [listMounted,    setListMounted]    = useState(isMobile)
  const [nurtureMounted, setNurtureMounted] = useState(false)

  // Keep viewMode in sync when screen rotates between mobile/desktop
  useEffect(() => {
  ...
  const onDragStart = useCallback((e, leadId) => {
    ...
    e.dataTransfer.effectAllowed = 'move'; e.dataTransfer.setData('text/plain', leadId)
  }, [isAffiliate])
  const onDragOver = useCallback((e, stageKey) => {
    if (isAffiliate) return
    e.preventDefault(); e.dataTransfer.dropEffect = 'move'; setDragTarget(stageKey)
  }, [isAffiliate])
  ...
  return (
    <div style={{ padding: isMobile ? '16px' : '28px', minHeight: 'calc(100vh - 60px)' }}>
      ...
      {viewMode !== 'nurture' && (
        <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
          <input
            type="text" placeholder="Search name, business, email…"
            value={filterSearch} onChange={e => setFilterSearch(e.target.value)}
            style={{ border: `1.5px solid ${ds.border}`, borderRadius: ds.radius.md, padding: '8px 14px', fontSize: 13, color: ds.dark, fontFamily: ds.fontDm, background: 'white', outline: 'none', width: isMobile ? '100%' : 240, minHeight: 40 }}
          />
          {!isMobile && (
            <>
...
function KanbanCard({ lead, onOpen, onDragStart, onDragEnd, isMoving, canDrag, attention }) {
  ...
  return (
    <div draggable={canDrag} onDragStart={canDrag ? onDragStart : undefined} onDragEnd={canDrag ? onDragEnd : undefined} onClick={onOpen}
      style={{ background: 'white', border: `1px solid ${badges.length > 0 ? '#FED7AA' : ds.border}`, borderRadius: ds.radius.md, padding: 12, marginBottom: 9, cursor: isMoving ? 'wait' : (canDrag ? 'grab' : 'pointer'), opacity: isMoving ? 0.4 : 1, boxShadow: ds.cardShadow, transition: 'all 0.15s', userSelect: 'none' }}
      onMouseEnter={e => { e.currentTarget.style.boxShadow = ds.hoverShadow; e.currentTarget.style.borderColor = ds.teal }}
      onMouseLeave={e => { e.currentTarget.style.boxShadow = ds.cardShadow; e.currentTarget.style.borderColor = badges.length > 0 ? '#FED7AA' : ds.border }}>
      ...
      {lead.business_name && <p style={{ color: ds.gray, fontSize: 11.5, margin: '0 0 7px' }}>{lead.business_name}</p>}
