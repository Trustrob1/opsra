...
export default function SubscriptionList({ user, onSelect, externalTick = 0 }) {
  ...
  const [confirmTarget, setConfirmTarget]   = useState(null)  // subscription id for quick confirm
  ...
  const handleApply = useCallback(() => {
    ...
    applyFilters(f)
  }, [filterStatus, filterPlan, filterWindow, applyFilters])
  ...
  return (
    <div>
      ...
      {/* ── Filter bar ────────────────────────────────────────────────────── */}
      <div style={{ background: 'white', borderRadius: 12, padding: '16px 20px', border: '1px solid #e2ecf0', marginBottom: 20, display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
