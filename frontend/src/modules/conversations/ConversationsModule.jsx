 ...
 *
 * Shows all WhatsApp (and future Instagram) conversations for:
 *   - Scoped roles (sales_agent etc.): only their assigned leads + customers
 *   - Admin / Owner / Ops Manager: all conversations across all reps
 *
 * Layout:
 *   Desktop: two-panel (conversation list left, thread right)
 *   Mobile PWA: single panel — list view by default, tap row to open
 *               full-screen thread, back arrow to return to list.
 *
 * Channels: whatsapp (live), instagram (Gap-3 — shows icon, no data yet)
 *
...
const CHANNEL = {
  whatsapp:  { label: 'WhatsApp',  icon: '💬', color: '#25D366', bg: '#E8F8EE', dot: '#25D366' },
  instagram: { label: 'Instagram', icon: '📷', color: '#C13584', bg: '#FCE4EC', dot: '#C13584' },
}
...
export default function ConversationsModule() {
  ...
  // Mobile panel state
  const [panel, setPanel] = useState('list')   // 'list' | 'thread'
  ...
  const loadMessages = useCallback(() => {
    ...
    Promise.resolve(fetchPromise)
      .then(res => {
        // leads.service:     { success, data: { items } }
        // whatsapp.service:  axios res → res.data.data.items
...
function ListPanel({
  conversations, allCount, loading, error,
  search, onSearch,
  channelFilter, onChannelFilter,
  typeFilter, onTypeFilter,
  unreadOnly, onUnreadOnly,
  active, onSelect,
  totalUnread,
}) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '14px 16px 10px', borderBottom: `1px solid ${ds.border}`, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 11 }}>
          <span style={{ fontFamily: ds.fontSyne, fontWeight: 700, fontSize: 16, color: ds.dark }}>
            Conversations
          </span>
          {totalUnread > 0 && (
            <span style={{
        ...
        <div style={{ position: 'relative', marginBottom: 9 }}>
          ...
          <input
            ...
            style={{
              ...
              border: `1.5px solid ${ds.border}`, borderRadius: 9,
              padding: '9px 10px 9px 32px',
              fontSize: 13, fontFamily: ds.fontDm,
              outline: 'none', background: ds.light, color: ds.dark,
            }}
          />
        </div>
        ...
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
          ...
          {/* Unread toggle */}
          <Chip active={unreadOnly} color='#E53E3E' onClick={() => onUnreadOnly(!unreadOnly)}>
            🔴 Unread
          </Chip>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
        {loading && (
          <div style={{ padding: '12px 0' }}>
            {[1,2,3,4,5,6].map(n => <ConvSkeleton key={n} />)}
          </div>
        )}

        {!loading && error && (
          <div style={{ padding: 28, textAlign: 'center', color: ds.red, fontSize: 13 }}>⚠ {error}</div>
        )}
        ...
        {!loading && !error && allCount > 0 && conversations.length === 0 && (
          <div style={{ padding: 32, textAlign: 'center', color: ds.gray, fontSize: 13 }}>
        ...
        {!loading && conversations.map(conv => (
          <ConvRow
            key={conv.contact_id}
            conv={conv}
            isActive={active?.contact_id === conv.contact_id}
            onSelect={onSelect}
          />
        ))}
      </div>
  ...
  return (
    <div
      ...
      style={{
        ...
        padding: '12px 16px',
        background: isActive ? ds.mint : hasNew ? '#FAFCFF' : '#fff',
        borderBottom: `1px solid ${ds.border}`,
        borderLeft: `3px solid ${isActive ? ds.teal : 'transparent'}`,
        cursor: 'pointer',
        transition: 'background 0.12s, border-color 0.12s',
        minHeight: 72,   // 44px min tap target + content
      }}
    >
      {/* Avatar */}
      <div style={{ position: 'relative', flexShrink: 0 }}>
        <div style={{
          ...
          background: isLead
            ? `linear-gradient(135deg, ${ds.accent} 0%, #C05A00 100%)`
            : `linear-gradient(135deg, ${ds.teal} 0%, ${ds.tealDark} 100%)`,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontFamily: ds.fontSyne, fontWeight: 700, fontSize: 15, color: '#fff',
        }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 3 }}>
          <span style={{
            fontFamily: ds.fontSyne, fontWeight: hasNew ? 700 : 600,
            fontSize: 13.5, color: ds.dark,
            overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
            flex: 1, marginRight: 8,
          }}>
            {conv.contact_name}
          </span>
          <span style={{ fontSize: 10.5, color: ds.gray, flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
          <span style={{
            fontSize: 12, color: hasNew ? ds.dark : ds.gray,
            fontWeight: hasNew ? 500 : 400,
            overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
            flex: 1,
          }}>
            {conv.last_message_direction === 'outbound' && (
              <span style={{ color: ds.teal, marginRight: 2, fontSize: 11 }}>↗ </span>
            )}
            {conv.last_message
              ? truncate(conv.last_message)
              : <em style={{ color: ds.gray }}>No messages yet</em>
            }
          </span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
            <span style={{
              ...
              background: isLead ? '#FFF3E0' : '#E0F4F6',
              color:      isLead ? '#C05A00' : ds.teal,
            }}>
              {isLead ? 'Lead' : 'CX'}
            </span>
            ...
            {hasNew && (
              <span style={{
                ...
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 10, fontWeight: 700, fontFamily: ds.fontSyne, padding: '0 4px',
              }}>
...
function ThreadPanel({ active, messages, loading, templates, threadRef, onSent, onBack }) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        {onBack && (
          <button
            ...
            style={{
              ...
              color: ds.teal, fontSize: 22, padding: '4px 6px 4px 0',
              lineHeight: 1, flexShrink: 0, minWidth: 36, minHeight: 44,
        ...
        <div style={{
          ...
          background: isLead
            ? `linear-gradient(135deg, ${ds.accent}, #C05A00)`
            : `linear-gradient(135deg, ${ds.teal}, ${ds.tealDark})`,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontFamily: ds.fontSyne, fontWeight: 700, fontSize: 14, color: '#fff',
        }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            fontFamily: ds.fontSyne, fontWeight: 700, fontSize: 14, color: ds.dark,
            overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
          }}>
            {active.contact_name}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 2, flexWrap: 'wrap' }}>
            <span style={{
              ...
              background: ch.bg, color: ch.color, fontWeight: 600, fontFamily: ds.fontSyne,
            }}>
              {ch.icon} {ch.label}
            </span>
            <span style={{
              fontSize: 10.5, padding: '1px 7px', borderRadius: 20, fontWeight: 600,
              fontFamily: ds.fontSyne,
              background: isLead ? '#FFF3E0' : '#E0F4F6',
              color:      isLead ? '#C05A00'  : ds.teal,
            }}>
            ...
            {active.phone && (
              <span style={{ fontSize: 11, color: ds.gray }}>{active.phone}</span>
            )}
          </div>
        </div>

        {active.assigned_name && (
          <div style={{ fontSize: 11, color: ds.gray, textAlign: 'right', flexShrink: 0 }}>
            <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: 1 }}>Rep</div>
            <div style={{ fontWeight: 600, color: ds.dark, fontSize: 12 }}>{active.assigned_name}</div>
          </div>
        )}
      </div>
      ...
      >
        {loading && (
          <div style={{ display: 'flex', justifyContent: 'center', padding: 40 }}>
            <div style={{
              width: 26, height: 26,
              border: `3px solid rgba(2,128,144,0.2)`,
      ...
      <div style={{ flexShrink: 0, borderTop: `1px solid ${ds.border}` }}>
        <MessageComposer
          {...(isLead
            ? { leadId: active.contact_id }
            : { customerId: active.contact_id }
          )}
          windowOpen={true}
          templates={templates}
          onSent={onSent}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        border:       `1px solid ${isOut ? '#B0DDB8' : ds.border}`,
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 13, color: ds.gray, textAlign: 'center', maxWidth: 260, lineHeight: 1.6 }}>
        {totalUnread > 0
          ? 'Choose a conversation on the left to reply'
          : 'Pick any conversation from the left panel to view the thread and send a message'}
      </div>
    </div>
  )
}
  ...
  return (
    <button
      ...
      style={{
        ...
        fontFamily: ds.fontSyne,
        background: active ? color : ds.light,
        color:      active ? '#fff' : ds.gray,
        transition: 'all 0.15s',
        minHeight: 28,    // comfortable tap target
      }}
    >
      {children}
    </button>
